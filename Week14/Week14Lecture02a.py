...
def main():
    ...
    numOfFeatures = secondLayerAfterPoolingRelu.shape[1] * secondLayerAfterPoolingRelu.shape[2]\
                    * secondLayerAfterPoolingRelu.shape[3]
    ...
    #optimizer = tf.train.AdamOptimizer(.0225).minimize(cost)
    optimizer = tf.train.GradientDescentOptimizer(.000005).minimize(cost)
    ...
    with tf.Session() as sess:
        ...
        for el in range(1000):
            sess.run(optimizer, feed_dict={xTensor: Xtrain[:1000].reshape([1000, 28, 28, 1]),
                                           yTensor: Ytrain[:1000]})
            print(el)
            if el == 0:
